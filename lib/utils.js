...
function getOrigin(ctx, options) {
  ...
  const orig = url.parse(ctx.origin, false);
  let finalPrefix = servicePrefix;
  if (supportSubDomain) {
    console.log(`getOrigin: uri.host=${uri.host}, orig.host=${orig.host}`);
    if (orig.host && orig.host !== uri.host && orig.host.includes(uri.host)) {
      finalPrefix = `${orig.protocol}//${orig.host}`;
    }
  }
  const hasSubpath = uri.pathname && uri.pathname !== '/' && orig.pathname.startsWith(uri.pathname);
  if (hasSubpath) {
    finalPrefix = finalPrefix.replace(uri.pathname, '');
  }
  return finalPrefix + orig.pathname + (querystring ? `?${querystring}` : '');
}
...
function getPath(ctx, name, options) {
  ...
  if (typeof ctx.origin !== 'string') throw new Error('utils.getPath ctx param must be set!');
  let path = '';
