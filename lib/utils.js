...
function getOrigin(ctx, options) {
  ...
  const uriObj = url.parse(ctx.originalUrl, false);
  let finalPrefix = servicePrefix;
  if (supportSubDomain) {
    const prefixObj = url.parse(servicePrefix, false);
    if (uriObj.host.includes(prefixObj.host)) {
      finalPrefix = `${uriObj.protocol}//${uriObj.host}`;
    }
  }
  return finalPrefix + uriObj.pathname + (querystring ? `?${querystring}` : '');
}
...
function getValidateUrl(ctx, options) {
  ...
  let path = '';
  let finalPrefix = servicePrefix;
  console.log(`supportSubDomain: `, supportSubDomain, ", servicePrefix: ", servicePrefix, ", originUrl: ", ctx.originalUrl);
  if (supportSubDomain) {
    const prefixObj = url.parse(servicePrefix, false);
...
function getPath(ctx, name, options) {
  ...
  switch (name) {
    ...
    case 'validate':
      ...
      console.log(`getPath: service: `, path);
      break;
