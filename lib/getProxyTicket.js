...
module.exports = function* (ctx, proxyOptions) {
  ...
  if (disableCache || !options.cache.enable || isMatchFilter || renew) {
    // Not to use cache
    disableCache ? logger.info('Enforce request pt, ignore cache') : logger.info('renew is true, refetch a new pt');
    try {
      const pt = yield requestPT(proxyPath);
      if (renew) {
        logger.info(`Refetch a new pt succeed, pt: ${pt}. Try store it in cache.`);
        yield ptStore.set(req, targetService, pt);
      }
      return pt;
    } catch (err) {
  ...
  // Use cache
  try {
    let pt = yield ptStore.get(req, targetService);
    if (pt) {
    ...
    try {
      ...
      if (pt) {
        ...
        if (typeof retryHandler === 'function') {
          return retryHandler(new Error(`Request for PT succeed, but response is invalid, response: ${response.body}`));
        }
