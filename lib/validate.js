...
function getLastUrl(ctx, options, logger, lastUrl) {
  if (is.function(options.redirect)) {
    ...
    if (is.string(customRedirectUrl)) {
...
function* validateTicket(ctx, options) {
  ...
  const query = {
    service: utils.getPath(ctx, 'service', options),
    ticket: ctx.query.ticket,
  };
  if (options.paths.proxyCallback) query.pgtUrl = utils.getPath(ctx, 'pgtUrl', options);
  const serviceValidateUrl = `${utils.getPath(ctx, 'serviceValidate', options)}?${queryString.stringify(query)}`;
...
function parseCasResponse(casBody, logger) {
  return new Promise((resolve, reject) => {
    xml2js(casBody, {
      ...
      const casResponse = {};
      Object.keys(success).forEach(casProperty => (casResponse[casProperty] = success[casProperty][0]));
...
module.exports = function* (ctx, afterHook, options) {
  ...
  if (!ticket) {
    logger.warn(`Can not find ticket in query, redirect to last url: ${lastUrl}`);
    yield afterHook();
    return ctx.redirect(lastUrl);
  }
