...
function* getProxyTicketThroughRestletReq(ctx, targetService, restletOptions) {
  ...
  let pgt = isUsingCache ? yield globalPGTStore.get(restletIntegrateRuleKey) : null;
  ...
  function* retryHandler(err) {
    ...
    yield globalPGTStore.remove(restletIntegrateRuleKey);
  ...
  yield globalPGTStore.set(restletIntegrateRuleKey, pgt);
