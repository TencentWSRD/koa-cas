...
function* getProxyTicketThroughRestletReq(ctx, targetService, restletOptions) {
  ...
  const path = utils.getPath(ctx, 'restletIntegration', options);
 ...
 */
// restletIntegrateRuleKey, restletIntegrateOption, doNotRetry
function* getProxyTicketThroughRestletReqDcache(ctx, targetService, restletOptions) {
  const options = this.options;
  const that = this;
  const logger = utils.getLogger(ctx, options);
  const doNotRetry = restletOptions.doNotRetry;
  const restletParams = restletOptions.params;
  const isUsingCache = restletOptions.cache;
  let restletIntegrateRuleKey = restletOptions.name;// 兼容之前，使用name作为cache key
  const dcachePgtStore = restletOptions.restletCache.cache;
  if (restletOptions.getRestletIntegrateRuleKey && typeof restletOptions.getRestletIntegrateRuleKey === 'function') {
    restletIntegrateRuleKey = restletOptions.getRestletIntegrateRuleKey(ctx);
  }
  if (!isUsingCache) logger.info('Don\'t use cached pgt by options.getProxyTicketThroughRestletReqDcache config.');
  let pgt = null;
  if (isUsingCache) {
  ...
  function* retryHandler(err) {
    ...
    // Set doNotRetry=true, retry once, no more.
    return yield getProxyTicketThroughRestletReqDcache.call(that, ctx, targetService, {
      name: restletIntegrateRuleKey,
      params: restletParams,
      doNotRetry: true,
      etRestletIntegrateRuleKey: restletOptions.getRestletIntegrateRuleKey,
  ...
  logger.info('Parse pgtId from response succeed, pgt: ', pgt);
  if (isUsingCache) dcachePgtStore.set(restletIntegrateRuleKey, pgt);
...
module.exports.getProxyTicketThroughRestletReq = getProxyTicketThroughRestletReq;
module.exports.getProxyTicketThroughRestletReqDcache = getProxyTicketThroughRestletReqDcache;
