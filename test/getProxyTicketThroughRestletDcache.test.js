...
const GlobalStoreDcache = require('./lib/globalStoreDcache');

const globalPGTDcacheStore = new GlobalStoreDcache();

describe('利用restlet integration decache访问正常', function() {
  ...
  beforeEach(function(done) {
    ...
    casClientFactory(casClientApp, {
      ...
      getRestletIntegrateRuleKey: (ctx) => {
        return 'rtxname';
      },
      logger,
    }, {
    ...
    hookAfterCasConfig = function* (ctx, next) {
      if (ctx.path === '/restlet') {
        if (ctx.query && ctx.query.time) {
          const cachedPgt = yield globalPGTDcacheStore.get('rtxname');
  ...
  it('未登陆下, 配置restletIntegration, 命中规则, 乱设一个pgt在globalStore, 能够自动重试并重新获取pgt, 然后获取pt', co.wrap(function*(done) {
    yield globalPGTDcacheStore.set('rtxname', 'some invalid pgt');
  ...
  it('未登陆下, 配置restletIntegration, 命中规则, 乱设一个pgt在globalStore, 获取pt失败, 但能够自动重试并重新获取pgt, 但是再次获取pt还是失败, 直接退出不再重试', co.wrap(function*(done) {
    yield globalPGTDcacheStore.set('rtxname', 'some invalid pgt');
    let invalidPgt;
    ...
    hookAfterCasConfig = function* (ctx, next) {
      if (ctx.path === '/restlet') {
        ...
        try {
          ...
          const validPgt = yield globalPGTDcacheStore.get('rtxname');
          expect(validPgt).to.not.equal(invalidPgt);
          ctx.body = pt;
        } catch (err) {
          ctx.status = 401;
